r"""Public ETAK dashboard server with optional Google Drive CSV download.

Render start command:
    python backend/etak_public_server_drive.py --csv data/etak_buildings_index.csv --host 0.0.0.0 --port $PORT

If data/etak_buildings_index.csv is missing, set this Render environment variable:
    ETAK_CSV_URL=https://drive.google.com/file/d/YOUR_FILE_ID/view?usp=sharing
"""
...
def ensure_csv(csv_path: Path, csv_url: str | None) -> Path | None:
    ...
    print(f"CSV not found at {csv_path}. Downloading ETAK CSV from {download_url}", file=sys.stderr)
    with urlopen(download_url, timeout=180) as response, target.open("wb") as handle:
        while True:
...
def main() -> int:
    ...
    if not csv_path:
        ...
        print("Set ETAK_CSV_URL to a public Google Drive sharing link, or upload data/etak_buildings_index.csv.", file=sys.stderr)
        return 2
    ...
    started = time.time()
    index = BuildingIndex.load(csv_path)
